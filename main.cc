...
//----------------------------------------------------------------------------
// Declaring all the objects used in the game
...
// Declaring variables used by the camera
...
void init()
{
  ...
  vec4 buildingPositions4[4] = {
    vec4(-0.75, 0.0001, 2.25, 1.0), vec4(-0.75, 0.0001, 3.75, 1.0),
    vec4(-0.75, 0.0001, 5.25, 1.0), vec4(-0.75, 0.0001, 6.75, 1.0)
  };
  ...
  // Initialize the plane and car objects
...
void display(void)
{
  ...
  // Drawing all the objects when display is called
  plane->draw();
  car->draw();
...
void cameraAndMovement(int key, int x, int y) {
  ...
  switch (key) {
    ...
    //moveLeft: rotates the object -90 degrees and updates current direction accordingly.
    //moveRight: rotates the object 90 degrees and updates current direction accordingly.
    //moveForward: translates the object according to the current direction.
    //moveBackward: translates the object according to the current direction.
...
// Timer function for timing of light change
void timer(int val) {
...
int main( int argc, char **argv )
{
  ...
  // Deletes all objects at end as cleanup
